...
props:{defaultValue:{type:Number, "default":powerFunctions.getCurrentPower()}, min:{type:Number, "default":20}, max:{type:Number, "default":100}, timerStep:{type:Number, "default":1}, interval:{type:Number, "default":1000}, thresholds:{type:Array, "default":[{threshold:30, value:1}, {threshold:100, value:10}]}}, data:function() {
...
new Vue({el:"#app", components:{InputSpinner:InputSpinner}, data:{currentVal:powerFunctions.getCurrentPower(), min:20, max:100, interval:1000, timerStep:5, thresholds:[{threshold:30, value:1}, {threshold:100, value:10}]}, methods:{updateValue:function(val) {
  this.currentVal = val;
  ...
  this.thresholds = this.thresholds.map(function(item, index) {
    if (thresholdIndex === index) {
      return {threshold:evt.target.value, value:item.value};
    }
    return item;
  });
  ...
  this.thresholds = this.thresholds.map(function(item, index) {
    if (thresholdIndex === index) {
      return {threshold:item.threshold, value:evt.target.value};
    }
    return item;
  });
}}});
...
$(document).ready(function() {
  ...
  try {
    $("body").ripples({resolution:512, dropRadius:10, perturbance:0.14,});
  } catch (e) {
    console.log(".error");
  }
  ripples();
...
var checkPowerFunctions = setInterval(function() {
  if (typeof powerFunctions !== "undefined") {
    clearInterval(checkPowerFunctions);
    console.log("powerFunctions zosta\u0142o zdefiniowane");
    var lang = powerFunctions.getDeviceLang();
    document.getElementById("LangElement").lang = lang;
    console.log("lang");
    console.log(lang);
  } else {
    console.log("czekam na za\u0142adowanie powerFunctions...");
  }
}, 100);
var knobEnabled = false;
var number = powerFunctions.getCurrentPower();
...
$(document).ready(function() {
  var currentLang = "pl";
  var pumpOffText = document.querySelector("#text-visability p");
  var appContent = document.querySelector(".app__content#IDapp__content");
  appContent.style.display = "none";
  knobEnabled = powerFunctions.isPumpEnabled();
...
function iosUpdate() {
  window.webkit.messageHandlers.savePower.postMessage({power:number + "", enabled:knobEnabled + ""});
}
